...
class NT_Xent_GPU(nn.Module):
    def __init__(self, batch_size, temperature, device="cuda", world_size=1):
        super(NT_Xent_GPU, self).__init__()
        self.batch_size = batch_size
        self.temperature = temperature
        self.world_size = world_size
        self.device = device

        # Build and register mask once
        self.mask = self._create_mask().to(device)
        self.labels = torch.zeros(2 * batch_size * world_size, dtype=torch.long, device=device)
